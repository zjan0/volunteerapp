...
$sql="SELECT *  from organization where status=:status";
if($sql)
{
$stmt=$userpdo->prepare($sql);
$stmt->execute(["status"=>"nerozhodnuto"]);
$events=$stmt->fetchAll();
}
if(($_SERVER['REQUEST_METHOD']=== 'POST')&&isset($_POST['zamitnuti']))
{
  $email = $_POST['organization_email'];
  $sqlvo="DELETE FROM organization where organization_email=:email";
  $stmtvo=$userpdo->prepare($sqlvo);
  $stmtvo->execute(["email"=>$email]);
}
...
<div class="lg:w-1/2">
        <h1 class="text-3xl font-bold text-black mb-8">Možné organizace</h1>
        <div class="grid grid-cols-1 gap-8">
          ...
          <input type="hidden" name="organization_email" value="<?= $event['organization_email'] ?>" />
            <button
                name="prijmuti"
                type="submit"
          ...
          <input type="hidden" name="organization_email" value="<?= $event['organization_email'] ?>" />
            <button
                ...
                zamitnout
                    ...
                    <?php endforeach; ?>
            <?php else: ?>
                <p class="text-gray-400">Žádné nové organizace</p>
            <?php endif; ?>
        </div>
    </div>
</body>
