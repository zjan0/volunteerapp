...
{
  $email = $_POST['organization_id'];
  $sqlvoa="SELECT * FROM organization_event where organization_id=:id";
  $stmtvoa=$userpdo->prepare($sqlvoa);
  $stmtvoa->execute(["id"=>$email]);
  $sqlvoc="DELETE FROM volunteer_event where organization_key=:id";
  $stmtvoc=$userpdo->prepare($sqlvoc);
  $stmtvoc->execute(["id"=>$sqlvoa['event_id']]);
  $sqlvo="DELETE FROM organization where organization_id=:id";
  $stmtvo=$userpdo->prepare($sqlvo);
  $stmtvo->execute(["id"=>$sqlvoa['event_id']]);
  $sqlvob="DELETE FROM organization_event where organization_key=:id";
  $stmtvob=$userpdo->prepare($sqlvob);
  $stmtvob->execute(["id"=>$email]);
}
...
<div class="lg:w-1/2">
        <h1 class="text-3xl font-bold text-black mb-8">Všechny organizace</h1>
        ...
        <form action="admin.php" method="post">
          <input type="hidden" name="organization_email" value="<?= $event['organization_id'] ?>" />
            <button
                name="zamitnuti"
                type="submit"
                ...
                smazání organizace
                    ...
                    <?php endforeach; ?>
            <?php else: ?>
                <p class="text-gray-400">Žádné organizace</p>
            <?php endif; ?>
        </div>
    </div>
</body>
