...
if(isset($_POST['uložit_jméno'])){
  if(!empty($_POST["username"]))
  {
    ...
    {
      $error="jméno dobrovolníka nemůže být prázdné";
    }
}
...
if(!empty($_POST["password"]))
  {
    ...
    $volunteer_password=password_hash($password,PASSWORD_BCRYPT);
    $akce="ulozit_heslo";
    $newname="UPDATE volunteer set volunteer_password=:volunteer_password where volunteer_id=$myuser";
    $stmt=$userpdo->prepare($newname); 
    $stmt->execute(["volunteer_password"=>$volunteer_password]);
  }
  else
    {
      $error="heslo dobrovolníka nemůže být prázdné";
    }
}
}
?>
...
<div class="mt-6">
   <?php if($akce!="zmenit_jmeno"&&$akce!="zmenit_heslo"){?>
        <form action="volstat.php" method="post">
