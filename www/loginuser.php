...
else
{
    ...
    $users=$stmt->fetch();
    if(!$users)
    {/*echo'<div>uzivatel neexistuje</div>';*/}
    else
    {
...
<body>
  ...
  <div class="sm:mx-auto sm:w-full sm:max-w-sm">
    ...
    <h2 class="mt-10 text-center text-2xl/9 font-bold tracking-tight text-black">Přihlásit dobrovolníka</h2>
    <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg">
  <?php echo $error ?>
</div>
  </div>
  <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
    <form action="loginuser.php" method="post"  class="space-y-6">
        <div>
        <label for="name"class="block text-sm/6 font-medium text-black-100">email dobrovolníka</label>
        <div class="mt-2">
        <input type="text"name="email" value="" id="email" placeholder="dobrovolnik@gmail.com" class="block w-full rounded-md bg-white/5 px-3 py-1.5 text-base text-black outline-1 -outline-offset-1 outline-white/10 placeholder:text-black-500 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500 sm:text-sm/6">
        </div></div>
        <div>
